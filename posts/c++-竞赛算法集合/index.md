...
#include<cmath>
#include<cstdio>
#include<cstdlib>    // 随机数生成 random
#include<ctime>
#include<iostream>
#include<sstream>
#include<iomanip>    // 格式化 right 和 setw(width)
...
```cpp
// 一维数组
fill(arr, arr + sizeof(arr), value);
fill_n(arr, sizeof(arr), value);
// 二维数组，只能填充 0 或 -1
memset(arr, 0, sizeof(arr));
for (int i = 0; i < row; i++)
    fill(a[i], a[i] + sizeof(a[i]), value);
```
